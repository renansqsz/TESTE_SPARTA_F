...
import { Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Typography, Box, MenuItem, Select, FormControl, CircularProgress, } from "@mui/material";
...
const dashSindra: React.FC = () => {
  ...
  const allVariaveis = Array.from(
    new Set(dados.map((d) => d.D2N).filter((v) => v && v.trim() !== ""))
  );
  const allPeriodos = Array.from(
    new Set(dados.map((d) => d.D3N).filter((p) => p && p.trim() !== ""))
  );
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box display="flex" flexDirection="column" alignItems="flex-start" mt={-10} mb={12}>
        <Typography variant="subtitle1" sx={{ fontWeight: "bold", color: "#32588b" }}>
          CATÁLOGO DE VALORES - SINDRA
        </Typography>
        <Typography variant="body2" sx={{ fontWeight: "medium", color: "#32588b" }}>
          Dados sobre indicadores econômicos
        </Typography>
      </Box>
                ...
                {filteredData.map((item, idx) => {
                  ...
                  return (
                    <TableRow key={idx} hover>
                      ...
                      <TableCell>{capitalize(item.D3N)}</TableCell>
